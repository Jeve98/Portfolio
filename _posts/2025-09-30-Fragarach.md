...
리팩토링을 거친 덕분에 새로운 패턴의 추가는 기획 의도에 맞게 우선순위, 패턴 명, 쿨타임만 해당 변수에 추가하는 것으로 충분해졌습니다. 즉, 추상화를 통해 직관적인 패턴의 추가/제거가 가능해졌으며 개발자가 한번에 확인해야 할 코드 양을 줄이는 것으로 가독성을 높이고 휴먼 에러를 줄일 수 있었습니다.
...
SSAFY 명세에 따라 SQLite3를 사용하여 Rest-API 서버를 구축했습니다. 하지만 프로젝트 진행 중 SQLite3가 가지는 write-lock 기반의 동시성 한계를 인지했습니다. 기존의 구조를 수정하는 것도 한 방법이었지만, 그전에 저희 게임에서 DB에 빈번한 쓰기 작업이 필요한 지를 먼저 분석했습니다. 다행히 1인용 웹 게임이었기에 실제 플레이 시 대부분의 요청은 데이터 조회였고 데이터 쓰기 요청은 회원가입과 저장 시점에서만 발생했습니다. 뿐만 아니라 배포 및 테스트를 해줄 인원이 SSAFY 서울 캠퍼스로 한정되어 있었기 때문에, 모든 이들이 한번에 회원가입 혹은 게임을 저장해도 1000여건이라는 비교적 처리 가능한 수준의 데이터 쓰기 요청이 최대였기 때문에 SQLite3로도 충분히 대응이 가능하다고 생각했습니다.

### 바이브 코딩된 비정형 구조 재정비

함께 프로젝트를 진행했던 팀원은 바이브 코더였기 때문에 각종 기능을 AI를 활용하여 개발했습니다. 이 과정에서 명확한 컨벤션과 컨텍스트가 정해져있지 않았기에 코드의 coupling이 굉장히 높은 상황이었습니다. 버그가 발생하더라도 어느 시점에 로그를 찍어서 확인해야할지 조차 명확하지 않은 상황이었고, 해당 사태를 벗어나고자 다시금 AI에게 의존하면 더 큰 문제가 추후에 발생할 것이 자명했습니다. 이에 전체 프로젝트 구조를 그림 그려가며 분석하기 시작했습니다. 또한 이를 기반으로 팀원이 구현한 코드가 어떤 로직으로 동작하는지 주석을 포함하여 정리하였습니다.

예를 들어, 획득한 스킬의 레벨을 올려 스킬을 강화하는 시스템이 추가됐을 때는 어떤 스킬은 정상적으로 시스템이 적용되지만 어떤 스킬은 아무런 변화가 없는 버그가 발생한 적 있었습니다. 당시에 코드를 분석해본 결과, 일부 스킬은 scene 자체에 로직이 바인딩 되어있었고 일부는 별도의 스킬 관련 코드로 분화되어있는 것을 발견할 수 있었습니다. 문제는 후자의 경우도 전체 스킬 구조를 짜둔 부모 코드를 상속 받아 레벨 별 성장치가 적용된 스킬이 있는가 하면, 상속은 받았으나 내부에 별도의 로직을 추가하여 해당 기능을 사용할 수 없는 경우도 있었습니다.

이러한 경우에 각 시스템이 어떻게 연관되어있고 어느 부분에서 문제가 발생되었으며 이를 어떻게 해결할 수 있을 지를 구조화 된 그림과 상세한 주석을 달아둔 코드와 함께 문서화하여 팀원에게 제공하였고 추가적으로 기본적인 리팩토링 방안을 설명해주는 것으로 팀원이 스스로 문제를 해결할 수 있도록 하였습니다.
