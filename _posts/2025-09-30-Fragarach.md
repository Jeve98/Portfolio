...
함께 프로젝트를 진행했던 팀원은 바이브 코더였기 때문에 각종 기능을 AI를 활용하여 개발했습니다. 이 과정에서 명확한 컨벤션과 컨텍스트가 정해져있지 않았기에 코드의 coupling이 굉장히 높은 상황이었습니다. 또한 AI가 생성하는 막대한 양의 코드로 인해 버그가 발생하더라도 그 원인을 분석하기가 굉장히 힘들었습니다. 하지만 해당 사태를 벗어나고자 다시금 AI에 의존하면 더 큰 문제가 추후에 발생할 것이 자명했습니다. 이에 전체 프로젝트 구조를 그림 그려가며 분석하기 시작했습니다. 또한 이를 기반으로 팀원이 구현한 코드가 어떤 로직으로 동작하는지 주석을 포함하여 정리하였습니다.

예를 들어, 획득한 스킬의 레벨을 올려 스킬을 강화하는 시스템이 추가됐을 때는 어떤 스킬은 정상적으로 시스템이 적용되지만 어떤 스킬은 아무런 변화가 없는 버그가 발생한 적 있었습니다. 당시에 코드를 분석해본 결과, 일부 스킬은 scene 자체에 로직이 바인딩 되어있었고 일부는 별도의 스킬 관련 코드로 분화되어있는 것을 발견할 수 있었습니다. 문제는 후자의 경우도 전체 스킬 구조를 짜둔 부모 코드를 상속 받아 레벨 별 성장치가 적용된 스킬이 있는가 하면, 상속은 받았으나 내부에서 새로운 로직을 추가하여 해당 함수를 사용하지 않는 경우도 있었습니다.
...
또 다른 사례로는 스킬 획득 메커니즘이 특정 레벨 도달 시 자동 해금되는 것에서 특정 레벨 도달 및 특정 스킬 획득으로 변경된 이후 발생한 버그가 있습니다. UI나 적용되는 스탯을 역분석 했을 때 레벨은 정상적으로 적용되고 있었으며 신규 스킬 해금을 위한 선행 스킬의 획득 역시도 문제가 없었으나, 첫 스킬 해금 이후 맵을 넘어가거나 재로그인 했을 때 해금한 스킬이 미해금 상태로 돌아가는 버그였습니다. 심지어 해당 스킬을 스킬 슬롯에 등록해둔 경우, 미해금 상태로 출력되고 UI와의 상호작용도 안되는 상황임에도 스킬 사용 자체는 가능했습니다.

해당 버그는 막대한 양의 AI 코드로 인해 로그를 찍기에도 누락되는 부분이 많았고 로그에서도 큰 문제가 없었습니다. 이에 해당 문제는 프로젝트 전체에서 레벨 달성과 관련된 함수와 변수, 선행 스킬 획득과 관련된 함수와 변수를 검색하여 흐름을 역행하며 분석하였습니다. 문제는 Vue를 통해 Phaser 엔진을 로드해올 때, API로 전송 받는 캐릭터 데이터가 없을 경우를 대비하여 더미로써 생성해둔 레벨 1 계정에 있었습니다. 해당 계정값에 API로 획득한 데이터가 덮어씌워지는 방식이었는데, 레벨을 덮어쓰는 코드가 누락되어 UI를 보여주고 상호작용을 담당하는 Vue 입장에서는 레벨 도달 조건이 해소되지 않았기 때문에 버그가 발생했었습니다.

해당 버그 해결 후에는 흐름은 복잡하지 않으나 코드 양이 많아 디버깅이 힘든 코드들을 각 함수와 변수가 어떤 흐름으로 동작하는지 별도로 정리하여 팀원과 공유하고 이를 바탕으로 버그 발생 시 의심되는 부분을 빠르게 확인하고 누락 없이 디버깅이 가능하도록 하였습니다.
