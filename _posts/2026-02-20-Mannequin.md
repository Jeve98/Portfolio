...
summary: |
  ...
  - Jitter 완화 작업
ssafy: 본 프로젝트는 삼성 청년 SW·AI 아카데미(SSAFY)에서 진행한 팀 프로젝트로, 소스코드의 전문을 외부에 공개하는 것이 불가능하여 일부 핵심 로직만 발췌하여 작성했습니다.
---
...
<div style="display: flex; gap: 25px; margin-bottom: 35px; align-items: center;">
  ...
  <div style="flex: 1.2;">
    <p style="margin: 0; font-size: 15px; color: #555; line-height: 1.6;">
      더미 설치 스킬의 경우, 전이, 교란에 사용되는 더미 마네킹을 맵에 설치하는 스킬로 정해진 구역에만 설치가 가능하도록 설계하였습니다. 일정 거리 내에서만 설치가 가능해야하므로 설치 가능한 구역을 리스트로 보유하였다가 스킬 사용 시에 이를 순회하며 가장 가까운 더미를 찾은 뒤 해당 더미와의 거리와 설치 가능한 거리를 비교하는 방식으로 구현하였습니다.<br><br>
      Physics Overlap 기능을 활용할 수도 있었지만 전체 더미의 수가 15개로 적어 Physics Overlap의 오버헤드를 고려했을 때 효용성이 많이 떨어졌고, 유저의 플레이 방법에 따라 더미가 한 곳에 밀집할 수도 있었기 때문에 리스트 순회 방식을 채택했습니다.
    </p>
  </div>
</div>
...
즉, 각 기능마다 상속해야하는 클래스가 달랐고(NetworkBehaviour, MonoBehaviour) C#에서는 단일 상속만이 가능하기 때문에 틀을 추상 클래스로 생성하기 보다 **인터페이스**로 생성하는 것이 효율적이라고 생각하였습니다. 또한 부모/자식의 관계가 수직적으로 확장되는 것이 아니라 '상호작용'이라는 큰 틀에서 서로 다른 기능으로 수평적으로 확장되기 때문에 인터페이스 더욱 적합하다고 판단하였습니다.
...
유니티는 3D 프로젝트에 기본적으로 Frustum Culling을 적용하여 카메라 뷰 외부의 오브젝트를 렌더링하지 않도록 최적화를 진행합니다. 기본적으로 Frustum Culling만으로도 높은 수준의 렌더링 성능을 확보할 수 있지만, 해당 프로젝트에서는 AI로 구현한 3D mesh가 많았기에 **vertex가 복잡한 오브젝트가 많았고 맵의 크기도 상당**하기에 한계가 존재하였습니다.

이에 오브젝트가 다른 오브젝트에 의해 가려진 경우에도 렌더링 대상에서 제외하는 **Occlusion culling을 추가적으로 적용**하였습니다. 특히 해당 프로젝트에서는 카메라 뷰 내부에는 존재하지만 오브젝트에 의해 가려져 보이지 않는 경우가 많았기 때문에 보다 효율적일 것이라 판단하였습니다.

또한 약 한달이라는 짧은 프로젝트 기간을 고려하였을 때, 개별 오브젝트에 LOD를 적용하는 것보다 공간 자체에 Occlusion Culling을 추가적으로 적용하는 것이 **비용 대비 효과**가 높고 일정을 맞추기에 적합할 것이라 판단하였습니다.
